 ...
 *   L2 风格：[style_preset.part2_content]（PORTRAIT / THREE_VIEW）
 *   L3 [渲染后的 CharacterProfile]（PORTRAIT 必填；THREE_VIEW 有来源档案时注入）
...
export const assemble = (input: AssembleInput): AssembleOutput => {
  ...
  const sections = isThreeView
    ? [
        lead,
        "来源定妆照是唯一身份参考图。必须保持来源定妆照中的同一个人：相同人物、相同性别、相同年龄感、相同脸型五官、相同发型发色、相同服装颜色与服装结构、相同配饰。不得改性别，不得换成其他人物，不得把东方古装女性改成现代男性或欧美男性。",
  ...
  const negative_prompt = appendPortraitGenderNegative(resolveNegativePrompt({
    ...
    portraitBackgroundMode,
  }), profileApplicable ? input.profile : null);
